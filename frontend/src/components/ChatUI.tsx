...
function obterCarroSugerido(
  ...
  const comCarro = fontes.filter((f) => f.carroId !== null);
  if (comCarro.length === 0) return null;

  const contagem = new Map<number, { nome: string; total: number }>();
  for (const f of comCarro) {
    if (f.carroId === null) continue;
    const nome = f.secao.split("—")[0]?.trim() ?? "este carro";
    const atual = contagem.get(f.carroId);
    contagem.set(f.carroId, {
      nome,
